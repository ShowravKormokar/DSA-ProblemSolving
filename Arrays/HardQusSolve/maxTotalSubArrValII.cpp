...
int main()
{
    ...
    int k = 3;
    cout << sol.maxTotalValue(nums, k) << endl; // Output: 9
    return 0;
}

/*
TC: O(n log n + k log n) -> O(n log n) for building the sparse table and O(k log n) for processing the priority queue.
SC: O(n log n) for the sparse table and O(n) for the priority queue.
*/