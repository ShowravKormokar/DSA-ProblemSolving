// Leetcode problem 2492 -> Minimum Score of a Path Between Two Cities
#include <bits/stdc++.h>
using namespace std;

class Solution
{
public:
    int minScore(int n, vector<vector<int>> &roads)
    {
        int ans = INT_MAX;
        vector<vector<pair<int, int>>> graph(n);
        queue<int> q{{0}};
        vector<bool> seen(n);
        seen[0] = true;

        for (const vector<int> &r : roads)
        {
            const int u = r[0] - 1;
            const int v = r[1] - 1;
            const int distance = r[2];
            graph[u].emplace_back(v, distance);
            graph[v].emplace_back(u, distance);
        }

        while (!q.empty())
        {
            const int u = q.front();
            q.pop();
            for (const auto &[v, d] : graph[u])
            {
                ans = min(ans, d);
                if (seen[v])
                    continue;
                q.push(v);
                seen[v] = true;
            }
        }

        return ans;
    }
};

int main()
{
    Solution sol;
    int n = 4;
    vector<vector<int>> roads = {{1, 2, 9}, {2, 3, 6}, {2, 4, 5}, {1, 4, 7}};
    cout << sol.minScore(n, roads) << endl; // Output: 5
    return 0;
}

/*
TC: O(n + m), where n is the number of cities and m is the number of roads.
SC: O(n + m), for storing the graph and the seen array.
*/