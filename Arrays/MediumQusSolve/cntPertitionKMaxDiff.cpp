...
int main()
{
    ...
    int k = 2;
    cout << "Number of partitions: " << sol.countPartitions(nums, k) << endl;
    return 0;
}

/*
TC: O(N log N) due to multiset operations for each of the N elements.
SC: O(N) for dp and prefixSum arrays, plus O(N) in worst case for multiset.
*/