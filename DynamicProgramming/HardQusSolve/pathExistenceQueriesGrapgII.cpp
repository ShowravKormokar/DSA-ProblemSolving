...
int main()
{
    ...
    int maxDiff = 4;
    vector<vector<int>> queries = {{0, 1}, {1, 2}, {0, 4}, {2, 3}};
    vector<int> result = solution.pathExistenceQueries(n, nums, maxDiff, queries);
...
/*
TC: O(n log n + q log n), where n is the number of elements in nums and q is the number of queries.
SC: O(n log n), for storing the jump table and other auxiliary data structures.
*/